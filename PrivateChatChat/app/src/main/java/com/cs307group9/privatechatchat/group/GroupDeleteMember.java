...
public class GroupDeleteMember extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        back = (ImageButton) findViewById(R.id.groupDeleteBack);
    ...
    class ServerConnectThreadDelete implements Runnable {
        public void run() {
            ...
            try {
                ...
                new Thread(new DeleteUser()).start();
            } catch (UnknownHostException ex) {
                System.out.println("Server not found: " + ex.getMessage());
            } catch (IOException ex) {
                System.out.println("I/O Error: " + ex.getMessage());
    ...
    class DeleteUser implements Runnable {
        public void run() {
            try {
                output.writeObject("DeleteFromGroup");
                output.writeObject("" + cur_gid);
                output.writeObject("" + select_uid);

                Log.e("GroupAddMember", "DELETING...." + "[" + cur_gName + "]" + cur_gid + ": " + select_uid);
...
//                        if (response.equals("Success")){
                            final Toast toast = Toast.makeText(GroupDeleteMember.this, "Success", Toast.LENGTH_SHORT);
                            toast.show();
//                        }
//                        else {
    ...
    void UpdateUserList(int id, String name) {
        this.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                SimpleAdapter simpleAdapter = new SimpleAdapter(GroupDeleteMember.this, list_item, R.layout.friend_list_adapter,
                        new String[]{"name", "says", "image"}, new int[]{R.id.name, R.id.says, R.id.imgtou});
                listView = (ListView) findViewById(R.id.DeleteMemberList);
            ...
            private void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                ...
                new Thread(new ServerConnectThreadDelete()).start();
            }
        });
    }
